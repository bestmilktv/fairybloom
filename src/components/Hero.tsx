...
const Hero = () => {
  ...
  return (
    <section 
      ref={heroRef} 
      className={`relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-background via-secondary/30 to-accent/20 scroll-fade-in ${heroVisible ? 'visible' : ''}`}
    >
      {/* Background Image */}
      <div 
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce">
        <div className="w-6 h-10 border-2 border-luxury-foreground/50 rounded-full flex justify-center">
          <div className="w-1 h-3 bg-luxury-foreground/50 rounded-full mt-2 animate-pulse"></div>
        </div>
      </div>
    </section>
  );
